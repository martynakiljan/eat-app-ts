...
type ContextType = {
	basket: BasketItem[]
	addToBasket: (id: string, name: string, price: number, amount?: number) => void
	openModal: () => void
	open: boolean
	handleClose: () => void
	handleOpen: () => void
	increaseInBasket: (itemID: string, itemamount: number, itemPrice: number) => void
	decreaseInBasket: (itemID: string, itemamount: number, itemPrice: number) => void
	totalPrice: number
	emptyBasket: boolean
	removeFromBasket: (itemPrice: number, itemId: string) => void
...
export const BasketProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
	...
	//add to Basket //
	const addToBasket = (id: string, name: string, price: number) => {
	...
	//increase in Basket//
	const increaseInBasket = (itemID: string, itemAmount: number) => {
		...
		setBasketLength(updatedBasket.reduce((total, item) => total + item.amount, 0))
	}

	// decrease
	const decreaseInBasket = (itemId: string, itemAmount: number) => {
		const updatedBasket = basket.map(item => {
	...
	const calculateTotalPrice = (basket: BasketItem[], deliveryOption: string) => {
		let totalPrice = 0
		basket.forEach(item => {
	...
	// remove from Basket //
	const removeFromBasket = (itemPrice: number, itemId: string) => {
